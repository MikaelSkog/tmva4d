...
from tmva4d.losses.soft_dice import SoftDiceLoss
from tmva4d.loaders.dataloaders import Rescale, Flip, HFlip, VFlip
...
def get_class_weights(signal_type):
    ...
    ----------
    signal_type: str
        Supported: 'elevation_azimuth'

    RETURNS
    -------
    weights: numpy array
    """
    weight_path = TMVA4D_HOME / 'config_files'
    if signal_type in ('elevation_azimuth'):
        file_name = 'ea_weights.json'
    else:
        raise ValueError('Signal type {} is not supported.'.format(signal_type))
    file_path = weight_path / file_name
    with open(file_path, 'r') as fp:
        weights = json.load(fp)
    weights = np.array([weights['background'], weights['person']])
...
def normalize(data, signal_type, norm_type='local'):
    ...
    elif signal_type == 'elevation_azimuth':
        if norm_type == 'tvt':
            file_path = TMVA4D_HOME / 'config_files' / 'ea_stats_all.json'
        else:
            raise TypeError('Global type {} is not supported'.format(norm_type))
        with open(file_path, 'r') as fp:
            ea_stats = json.load(fp)
        min_value = torch.tensor(ea_stats['min_val'])
        max_value = torch.tensor(ea_stats['max_val'])

    elif signal_type == 'doppler_azimuth':
        if norm_type == 'tvt':
            file_path = TMVA4D_HOME / 'config_files' / 'da_stats_all.json'
        else:
            raise TypeError('Global type {} is not supported'.format(norm_type))
        with open(file_path, 'r') as fp:
            da_stats = json.load(fp)
        min_value = torch.tensor(da_stats['min_val'])
        max_value = torch.tensor(da_stats['max_val'])

    elif signal_type == 'elevation_doppler':
        if norm_type == 'tvt':
            file_path = TMVA4D_HOME / 'config_files' / 'ed_stats_all.json'
        else:
            raise TypeError('Global type {} is not supported'.format(norm_type))
        with open(file_path, 'r') as fp:
            ed_stats = json.load(fp)
        min_value = torch.tensor(ed_stats['min_val'])
        max_value = torch.tensor(ed_stats['max_val'])

    elif signal_type == 'elevation_range':
        if norm_type == 'tvt':
            file_path = TMVA4D_HOME / 'config_files' / 'er_stats_all.json'
        else:
            raise TypeError('Global type {} is not supported'.format(norm_type))
        with open(file_path, 'r') as fp:
            er_stats = json.load(fp)
        min_value = torch.tensor(er_stats['min_val'])
        max_value = torch.tensor(er_stats['max_val'])

    elif signal_type == 'range_azimuth':
        if norm_type == 'tvt':
            file_path = TMVA4D_HOME / 'config_files' / 'ra_stats_all.json'
        else:
            raise TypeError('Global type {} is not supported'.format(norm_type))
        with open(file_path, 'r') as fp:
